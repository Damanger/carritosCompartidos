...
fun ConductorFormDialog(onDismiss: () -> Unit, onSubmit: (Map<String, String>) -> Unit) {
    ...
    var sitio by remember { mutableStateOf("Otro") }
    ...
    val sitios = listOf(
        "Sitio Universitario", "23 de Julio", "Acatlima", "Otro"
    )

    var showDialog by remember { mutableStateOf(false) }

    val allFieldsValid = !correoError && !placasError && !telefonoError && sitio != "Otro" && nombre.isNotBlank()
            ...
            Column {
                ...
                Row(
                    ...
                    Button(
                        ...
                        Text("Sitio seleccionado:", color = Color.Black.copy(alpha = 0.75f))
                    }
                    Text(sitio)
                }
            }
    ...
    // Dialogo de selección de sitio
    if (showDialog) {
        SitioSelectionDialog(
            sitios = sitios,
            selectedSitio = sitio,
            onDismiss = { showDialog = false },
            onSitioSelected = { selectedSitio ->
                sitio = selectedSitio
                showDialog = false
            }
        )
...
@Composable
fun SitioSelectionDialog(
    sitios: List<String>,
    selectedSitio: String,
    onDismiss: () -> Unit,
    onSitioSelected: (String) -> Unit
) {
    AlertDialog(
        onDismissRequest = onDismiss,
        {  }
        ...
        text = {
            Column {
                Text(text = "Seleccione un Sitio")
                Spacer(modifier = Modifier.height(8.dp))
                sitios.forEach { option ->
                    Row(
                        verticalAlignment = Alignment.CenterVertically,
                        modifier = Modifier.clickable {
                            onSitioSelected(option)
                        }
                    ) {
                        RadioButton(
                            selected = (option == selectedSitio),
                            onClick = {
                                onSitioSelected(option)
                            }
                        )
        ...
        dismissButton = {
            Button(onClick = onDismiss, colors = ButtonDefaults.run { val buttonColors: ButtonColors =
                buttonColors(Color.Red.copy(alpha = 0.5f))
                buttonColors
            },) {
                Text("Cancelar", color = Color.Black.copy(alpha = 0.5f))
            }
        }
    )
}
